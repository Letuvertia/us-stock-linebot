...
sys.path.insert(0, os.path.join(os.path.dirname(_dir), 'market_data'))
...
def main():
    ...
    reuters_sid = get_news_spreadsheet_id('Reuters')
    sheets = get_news_sheets_service().spreadsheets().values()

    ticker_keywords = load_ticker_keywords()
    existing_urls = load_existing_urls(sheets, reuters_sid)
