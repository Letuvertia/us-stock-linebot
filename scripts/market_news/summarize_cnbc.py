...
1. **summary**：繁體中文摘要，嚴格限制在 150 字以內。
   - **格式**：以 [關鍵詞] 開頭，隨後緊接 3 個精煉短句。
   - **核心原則**：必須完全基於新聞中明確提及的客觀事實。請依新聞內容動態拆解為「事件、數據、直接反應」或「時地人、起因、結果」。如果新聞未提及未來風險，絕對禁止憑空捏造，請專注於還原新聞事件本身。

2. **related_stocks**：列出 2-3 檔最相關的 S&P 500 股票標的。
   - **核心原則**：此處請展現金融分析師的專業推論能力。請基於新聞內容，延伸評估該事件對相關產業或個股的連帶影響。
   - **屬性**：
     - ticker: 股票代碼。
     - reason: 關聯原因（請說明該新聞事件如何透過產業鏈、成本或市場情緒影響該個股）。
     - impact: 填入「利多」、「利空」或「中立」。

3. **語言**：使用台灣繁體中文金融用語（如：營收、地緣政治、避險、財報、個股）。

4. **輸出格式**：僅輸出標準 JSON 格式，不要有任何 Markdown 代碼塊（如 ```json）、額外解釋或思考過程。

【JSON 結構範例】：
{{
  "summary": "[地緣政治] 美伊衝突升級導致原油供應疑慮。目前油價飆升推升通膨預期。美方正密切觀察後續供需變化。",
  "related_stocks": [
    {{
      "ticker": "XOM",
      "reason": "原油價格上漲利於油氣開發商之產品售價與利潤率",
      "impact": "利多"
    }},
    {{
      "ticker": "DAL",
      "reason": "航油成本攀升將直接侵蝕航空公司第二季營運毛利",
      "impact": "利空"
    }}
  ]
}}
